...
class CheckerBoard(object):
    ...
    def draw_lines(self, row, col):
        ...
        while True:
            for event in pygame.event.get():
                if event.type == QUIT:
                    sys.exit()
                elif event.type == MOUSEBUTTONDOWN and gamefirst:
                ...
                elif event.type == MOUSEBUTTONDOWN and playerfirst:
                    mouse_down_x, mouse_down_y = event.pos
                    pos_real = self.check_mouse_down(mouse_down_x, mouse_down_y)
                    if pos_real is not None:
                        self.mouse_down_real.append(pos_real)
            ...
            for pos in [(col/5,row/5),(col/5,row/2), (col/5,row/5*5), (col/2,row/5),(col/2,row/2),
                    (col/2,row/5*5),(col/5*5,row/5),(col/5*5,row/2),(col/5*5,row/5*5)]:
    ...
    def take_one_step(self):
        ...
        if len(available_points) != 0:
            # pick one point
            print list(self.mouse_down_real)
